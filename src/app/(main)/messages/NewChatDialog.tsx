...
export default function NewChatDialog({
  ...
  const mutation = useMutation({
    ...
    onError(error) {
      ...
      toast({
        variant: "destructive",
        title: "Failed to create chat. Please try again.",
      });
    },
  });
  return (
        ...
        <div className="">
          ...
          <div className="h-96 overflow-y-auto">
            ...
            {isSuccess && !data?.users.length && !isFetching && (
              <p className="my-3 text-clip px-6 text-muted-foreground">
                No users found. Try searching for more.
              </p>
            )}
            {isFetching && <Loader2 className="mx-auto my-3 animate-spin" />}
            {isError && (
              <p className="my-3 px-6 text-center text-destructive">
